...
global.config = new JsonDB(
  new Config(path.join(__dirname, "..", "config.json"), true, true, "/")
);
global.constants = new JsonDB(
  new Config(path.join(__dirname, "..", "constants.json"), true, true, "/")
);
...
client.once("ready", async () => {
  ...
  client.users
    .fetch(config.getData("/creator_id"))
    .then((u) => u.send("🔄 Le bot a redemarré !"));
...
for (const file of commandFiles.admin) {
  import(toWindowsPath(path.join(admin_path, file))).then((command) => {
    client.commands.set(command.data.name, command);
  });
}
for (const file of commandFiles.everyone) {
  import(toWindowsPath(path.join(everyone_path, file))).then((command) => {
    client.commands.set(command.data.name, command);
  });
}
