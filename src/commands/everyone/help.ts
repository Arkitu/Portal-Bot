...
export async function execute(interaction: CommandInteraction) {
  ...
  if (opt_categorie) {
    help_embed.setAuthor({
      ...
      url: config.getData("/help_link"),
    });
    for (let cmd of constants.getData(
      `/helpCategories[${constants.getIndex(
        "/helpCategories",
        opt_categorie,
        "name"
      )}]/commands`
      ...
      help_embed.addFields({
        name: `\`/${cmd}\``,
        value: constants.getData(
          `/commands[${constants.getIndex(
            "/commands",
            cmd,
            "name"
          )}]/description`
    ...
    help_embed.setAuthor({
      name: `Aide de ${client.user.username}`,
      iconURL: client.user.avatarURL(),
      url: config.getData("/help_link"),
    });
