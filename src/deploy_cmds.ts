...
(async () => {
  ...
  for (let file of commandFiles.admin) {
    cmds.admin.push(
      (await (
        await import(toWindowsPath(join(path.admin, file)))
      ).data.toJSON()) as SlashCommandBuilderWithPerms
    );
  }
  for (let file of commandFiles.everyone) {
    cmds.everyone.push(
      (await (
        await import(toWindowsPath(join(path.everyone, file)))
      ).data.toJSON()) as SlashCommandBuilderWithPerms
    );
  }
  ...
  await rest.put(Routes.applicationCommands(config.getData("/app_id")), {
    body: cmds.everyone,
  });
  await rest.put(
    Routes.applicationGuildCommands(
      config.getData("/app_id"),
      config.getData("/admin_server_id")
    ),
    { body: cmds.admin }
  );
