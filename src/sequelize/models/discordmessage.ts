...
> {
   ...
   */
  static associate() {
    DiscordMessage.Message = db.models.DiscordMessage.belongsToMany(
      db.models.Message,
      { through: "DiscordMessageMessages" }
    );
    DiscordMessage.Channel = db.models.DiscordMessage.belongsTo(
      db.models.Channel
    );
  }
}
...
export function initModel() {
  DiscordMessage.init(
    initArgs as unknown as ModelAttributes<
      DiscordMessage,
      Optional<InferAttributes<DiscordMessage, { omit: never }>, never>
    >,
    {
      sequelize: db,
