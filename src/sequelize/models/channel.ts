...
export class Channel extends Model<InferAttributes<Channel>, InferCreationAttributes<Channel>> {
     ...
     */
    static associate() {
        Channel.Portal = db.models.Channel.belongsToMany(db.models.Portal, { through: "PortalChannels" });
        Channel.DiscordMessage = db.models.Channel.hasMany(db.models.DiscordMessage);
    }
}

export function initModel() {
    Channel.init(
        initArgs as unknown as ModelAttributes<Channel, Optional<InferAttributes<Channel, {omit: never;}>, never>>,
        {
            sequelize: db,
            modelName: 'Channel'
        }
